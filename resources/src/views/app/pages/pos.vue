  ...
  <div class="pos_page">
    <div class="container-fluid p-0 app-admin-wrap layout-sidebar-large clearfix" id="pos">
      ...
      <validation-observer ref="Add_payment">
        <b-modal id="Add_Payment" size="lg" hide-footer centered dialog-class="modal-custom-width">
          <!-- HEADER -->
         <template #modal-header>
          <div class="d-flex w-100 justify-content-between align-items-center">
            <!-- Left: Modal Title -->
            <h5 class="modal-title mb-0">{{ $t('Payment') }}</h5>

            <!-- Center: Grand Total -->
            <span class="h5 text-success mb-0">
              {{ currentUser.currency }} {{ GrandTotal.toFixed(2) }}
            </span>
            ...
            <b-button
              ...
              size="sm"
              class="close m-0 p-0"
              @click="$bvModal.hide('Add_Payment')"
              aria-label="Close"
            >
          ...
          <b-form @submit.prevent="Submit_Payment">
            ...
            <b-row>
              ...
              <b-col md="4">
                <b-card class="mb-3">
                  <b-list-group flush>
                    <!-- Total Paying with icon -->
                    <b-list-group-item class="d-flex justify-content-between align-items-center">
                      <div class="d-flex align-items-center">
                        <i class="i-Money-2 text-primary mr-2" style="font-size:1.2rem;"></i>
                        <span>{{ $t('TotalPaying') }}</span>
                      </div>
                      <strong>{{ currentUser.currency }} {{ totalPaid }}</strong>
                    </b-list-group-item>

                    <!-- Balance with icon -->
                    <b-list-group-item class="d-flex justify-content-between align-items-center">
                      <div class="d-flex align-items-center">
                        <i class="i-Money-Bag text-warning mr-2" style="font-size:1.2rem;"></i>
                        <span>{{ $t('Balance') }}</span>
                      </div>
                      <strong>{{ currentUser.currency }} {{ balance }}</strong>
                    </b-list-group-item>

                    <!-- Change Return with icon -->
                    <b-list-group-item class="d-flex justify-content-between align-items-center">
                      <div class="d-flex align-items-center">
                        <i class="i-Arrow-Back3 text-success mr-2" style="font-size:1.2rem;"></i>
                        <span>{{ $t('ChangeReturn') }}</span>
                      </div>
                      <strong>{{ currentUser.currency }} {{ changeReturn }}</strong>
                    </b-list-group-item>
                  </b-list-group>
                </b-card>
              </b-col>
              ...
              <!-- SPLIT-PAYMENT FORM -->
              <b-col md="8">
                <b-card class="mb-3" v-for="(p, idx) in paymentLines" :key="idx">
                    <!-- Header with remove icon -->
                    <template #header>
                      <div class="d-flex justify-content-between align-items-center">
                        <span>{{ $t('Payment') }} #{{ idx + 1 }}</span>
                        <i
                          v-if="idx > 0"
                          class="i-Remove text-danger"
                  ...
                  <b-row class="mb-4">
                    <b-col md="12">
                      <b-button block variant="outline-primary" @click="addPaymentLine">
                        + {{ $t('AddAnotherPaymentOption') }}
                      </b-button>
                    </b-col>
                  </b-row>
                ...
                <b-row class="mb-4">
                  <b-col md="12">
                    <b-card v-show="anyCreditCardUsed">
                      <!-- Show spinner while submitting -->
                      <div v-if="submit_showing_credit_card">
                      ...
                      <div v-if="displaySavedPaymentMethods && !submit_showing_credit_card">
                        <div class="mt-3">
                          <span class="mr-3">Saved Credit Card Info For This Client</span>
                          <b-button variant="outline-info" @click="show_new_credit_card()">
                            <i class="i-Two-Windows"></i> New Credit Card
                          </b-button>
                        </div>
                      ...
                      <div v-if="displayFormNewCard && !submit_showing_credit_card">
                        <form id="payment-form">
                          <label for="card-element" class="leading-7 text-sm text-gray-600">
                            ...
                            >
                              <i class="i-Two-Windows"></i> Use Saved Credit Card
                            </b-button>
                          </label>
                ...
                <!-- Popular Tendered -->
                <div class="mb-3">
                  <small class="text-muted">{{ $t('PopularTendered') }}</small>
                  <div class="d-flex flex-wrap mt-2">
                    <b-button
                      v-for="amt in [GrandTotal,15,20,50,100,200]"
                      :key="amt"
                    ...
                    >
                      {{ currentUser.currency }} {{ amt.toFixed(2) }}
                    </b-button>
                  </div>
                </div>
                ...
                >
                  <b-button variant="light" @click="appendDigit('1')">1</b-button>
                  <b-button variant="light" @click="appendDigit('2')">2</b-button>
                  <b-button variant="light" @click="appendDigit('3')">3</b-button>
                  <b-button variant="light" @click="appendDigit('4')">4</b-button>
                  <b-button variant="light" @click="appendDigit('5')">5</b-button>
                  <b-button variant="light" @click="appendDigit('6')">6</b-button>
                  <b-button variant="light" @click="appendDigit('7')">7</b-button>
                  <b-button variant="light" @click="appendDigit('8')">8</b-button>
                  <b-button variant="light" @click="appendDigit('9')">9</b-button>
                  <b-button variant="light" @click="clearInput">{{ $t('clear') }}</b-button>
                  <b-button variant="light" @click="appendDigit('0')">0</b-button>
                  <b-button variant="light" @click="backspace">
                    <i class="i-Backspace"></i>
                  </b-button>
                </div>

                <!-- Global Payment Note & Sale Note -->
                <b-row class="mb-4">
                  <b-col md="6">
                    <b-form-group :label="$t('Payment_note')">
                      <b-form-textarea
                        v-model="globalPaymentNote"
                        rows="3"
                      />
                    </b-form-group>
                  </b-col>
              ...
              <b-col md="6" class="mt-3">
                ...
                <b-button
                  ...
                  :disabled="totalPaid <= 0 || paymentProcessing"
                >
                  {{ $t('Pay') }}
                </b-button>
  ...
  Submit_Payment() {
    ...
    this.$refs.Add_payment.validate().then(success => {
      ...
      if (total <= 0) {
        ...
        this.makeToast(
          "warning",
          this.$t("TotalPaymentMustBeGreaterThanZero"),
          this.$t("Warning")
        );
        return;
      }
